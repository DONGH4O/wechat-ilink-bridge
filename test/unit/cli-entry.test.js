...
test("wxb M8 CLI entry emits JSON help", async () => {
  ...
  assert.equal(parsed.data.milestone, "M8");
  ...
  assert.equal(parsed.data.commands.send, "Send text to a user using cached context token");
...
test("wxb M8 CLI entry emits structured errors for unknown commands", async () => {
  const cliPath = path.resolve("src", "cli", "index.js");
